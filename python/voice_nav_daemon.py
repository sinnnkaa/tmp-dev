...
# Сколько ждать, пока процесс захвата уйдёт сам, прежде чем убивать его
# насмерть. Обоснование величины — там же, где она используется, в конце
# listen_and_transcribe(). Секунды хватает с огромным запасом: здоровый ffmpeg
# закрывается за миллисекунды, а больной не закроется никогда.
CAPTURE_EXIT_TIMEOUT = 1.0
...
def play_ready_tone():
    ...
    Громкость больше не переустанавливается здесь принудительно. Раньше перед
    каждым сигналом стоял `pactl set-sink-volume 100%` — задумывался как
    гарантия от "слишком тихо", а на живом тесте дал обратный эффект: "всегда
    очень громко, громкость не регулируется". Причина в частоте вызова —
    play_ready_tone звучит перед каждым захватом речи ("Слушаю адрес",
    подтверждение адреса), то есть по несколько раз за один заход к маршруту,
    и любая попытка человека убавить громкость на самой гарнитуре откатывалась
    обратно к максимуму на следующем же вызове. Громкость теперь целиком в
    руках пользователя (кнопки гарнитуры/телефона); амплитуда самого сигнала
    оставлена на 1.0 (пик без клиппинга) — это разговор о форме сигнала, не о
    громкости выходного устройства."""
...
def listen_and_transcribe(model, seconds):
    ...
    try:
        ...
        # Флаг снимается первым, до возни с процессом: если та свалится с
        # исключением, ядро иначе осталось бы навсегда с отложенными
        # предупреждениями — то есть немым.
        write_mic_open(False)

        # Дальше — уборка процесса захвата, и одного terminate() ей не хватает.
        # Наблюдение с платы: после ранней остановки (Vosk узнал фразу раньше,
        # чем истекли отведённые секунды) ffmpeg оставался жив спустя минуту и
        # держал микрофон гарнитуры открытым. Цикл выше к этому моменту читать
        # перестал, труба забилась, и процесс стоит в write(); обработчик
        # SIGTERM в ffmpeg только поднимает флаг, который проверяется между
        # кадрами, а прерванный сигналом write glibc просто перезапускает.
        # Поэтому сначала закрывается наш конец трубы — write падает с EPIPE и
        # расклинивает процесс, — и лишь потом ожидание с добиванием.
        #
        # Раньше промах ничем не грозил и потому не был виден: после диктовки
        # профиль возвращался в a2dp_sink, bluez_source исчезал физически и
        # уносил зависший захват с собой. В постоянном HFP исчезать нечему, и
        # такие процессы копятся по одному на диктовку, каждый со своим
        # открытым микрофоном.
        proc.terminate()
        try:
            proc.stdout.close()
            ...
            pass
        try:
            proc.wait(timeout=CAPTURE_EXIT_TIMEOUT)
        except subprocess.TimeoutExpired:
            print("[STT] Захват не завершился сам — убиваю принудительно.")
            proc.kill()
            proc.wait()
...
def navigation_worker(model, addresses_db, address_keys):
    ...
    # Первый манёвр объявляется сразу, а не по общему правилу цикла (там
    # звучит, только когда до него меньше TURN_ANNOUNCE_MAX_DIST). Живой тест:
    # маршрут построен, расстояние названо, а дальше тишина — потому что
    # первый поворот часто дальше 30м, а самый первый отрезок маршрута нередко
    # "прямо" (SILENT_DIRECTIONS), о котором цикл вообще не сообщает. Человек
    # после "маршрут построен" не слышит, в какую сторону вообще идти, пока не
    # пройдёт большую часть первого отрезка вслепую. Здесь — ровно одна фраза
    # об исходном направлении, до входа в цикл.
    if len(steps) > 1:
        first_maneuver = steps[1].get("maneuver", {})
        first_direction = first_maneuver.get("modifier", "straight")
        first_dist = int(steps[1].get("distance", 0))
        if is_announced(first_direction):
            speak(turn_phrase(first_dist, first_direction), sync=True)
            loc = first_maneuver.get("location", [0, 0])
            last_announced_loc = f"{loc[0]},{loc[1]}"
        else:
            speak("Идите прямо", sync=True)
