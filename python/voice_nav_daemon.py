...
def play_ready_tone():
    ...
    отдельный time.sleep() до запуска ffmpeg: свежий SCO-канал гарнитуры сам
    ещё секунду-другую "глухой" после подтверждения профиля (BT-модуль/
    наушники мьютят звук на старте линка), и короткий сигнал без этого запаса
    целиком проваливался в эту паузу — аplay рапортовал успешный старт
    воспроизведения, а физически ничего не звучало. Живой тест подтвердил: с
    запасом 0.35с сигнал слышен только со второй попытки подряд (к этому
    моменту с момента подтверждения профиля реально проходит больше времени,
    чем за один вызов) — то есть 0.35с мало именно для *первого* открытия
    линка в рамках сессии, запас увеличен до 1.0с. Тишина внутри потока (а не
