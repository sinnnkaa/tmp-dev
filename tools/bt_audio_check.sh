...
# Последним восстанавливается рабочий профиль, а не A2DP жёсткой строкой.
# Рабочим стал HFP (см. BLINDNAV_BT_PROFILE в voice_env.sh), и диагностика,
# уходящая с чужим профилем, — это диагностика, которая сама ломает то, что
# проверяет. bt_keeper вернул бы его за десять секунд, но эти десять секунд
# устройство звучало бы неправильно, а человек списал бы это на проверку.
probe "$BLINDNAV_BT_PROFILE" "$([ "$BLINDNAV_BT_PROFILE" = "handsfree_head_unit" ] && echo 16000 || echo 44100)"

echo "=== Итог ==="
echo "Рабочий профиль устройства — $BLINDNAV_BT_PROFILE, он восстановлен последним."
