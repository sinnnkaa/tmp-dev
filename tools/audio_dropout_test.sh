...
# --- 3. Холодный старт против тёплого -------------------------------------
# Отдельный признак, названный в поле дважды: помехи именно В НАЧАЛЕ фразы.
# module-suspend-on-idle усыпляет синк, а для bluez сон означает разбор
# A2DP-транспорта: следующая фраза начинается с переговоров AVDTP, разгона
# кодера и наполнения пустого буфера — всё поверх первых долей секунды речи.
# Для боевого устройства это основной режим: между репликами маршрута
# проходят минуты, то есть КАЖДАЯ фраза стартует холодной.
if announce "Прогон 3 из 4. ХОЛОДНЫЙ И ТЁПЛЫЙ СТАРТ. Слушайте именно НАЧАЛО фразы, два раза."; then
...
=== Как читать результат ===
  ...
  3: Б чище, чем А            -> вдобавок виноват подъём транспорта из сна.
                                  Лечится фоновой тишиной из bt_keeper;
                                  делать это стоит независимо от прочего.
EOF
